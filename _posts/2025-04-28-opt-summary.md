...
7. **Exact Matching**: In fact for some settings, some works has already shown that the lower and upper bounds are **exactly** the same, i.e., the two bounds are the same in both orders and constants, so big-O notations are unnecessary in these cases. In detail:
